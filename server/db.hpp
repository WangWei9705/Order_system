...
public:
    ...
    bool Select(Json::Value* orders) {
        // 构造SQL语句
        char SQL[1024*4] = {0};
        sprintf(SQL, "select order_id, table_id, time, dish_ids, state from order_table");

        // 执行SQl语句
        int ret = mysql_query(_mysql, SQL);
        if(ret != 0) {
            printf("select orders falied ! sql = %s, error = %s\n", 
                   SQL, mysql_error(_mysql));
            return false;
        }
        // 获取查询结果
    ...
    bool Insert(const Json::Value& order) {
        ...
        sprintf(SQL, "insert into order_table values(null, '%s', '%s', '%s', %d)",
            ...
            order["dish_ids"].asCString(), order["state"].asInt());
    ...
    bool UpdateState(const Json::Value& order) {
        ...
        int ret = mysql_query(_mysql, SQL);
        if(ret != 0) {
            printf("update falied ! sql = %s, error = %s\n",
                   SQL, mysql_error(_mysql));
            return false;
        }
        return true;

...
int main()
{
    ...
    // 获取客户端，匹配桌子id
    server.Get(("/table"),  [](const Request& req, 

                                            Response& resp) {
               // 从query_string中获取table_id 将其填入到页面指定位置
                const string& table_id = req.get_param_value("table_id");
                string html;
                printf("桌号为：%s\n", table_id.c_str());
                ...
                string html_out;
                // 将html中的{{table_id}}  替换为table_id
                StringUtil::Replace(html, "{{table_id}}", table_id, &html);

                // 将替换后的值写入html中
                resp.set_content(html_out, "test/html");
    }); 
    // 设置静态文件目录
